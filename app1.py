...
#     pages = ["🔍 Oportunidade de melhorias", "📋 Oportunidades Identificadas"]
...
def setup_navigation():
    ...
    # Incluímos todas as páginas na navegação
    pages = [
...
def main():
    ...
    with main_container:
        ...
        elif pages[st.session_state.current_page] == "📋 Diagnóstico Oportunidades":
            render_diagnostico_op()
        elif pages[st.session_state.current_page] == "📋 Planilha Final (Dir)":
            render_planilha_final_dir()
        elif pages[st.session_state.current_page] == "📋 Planilha Final (Op)":
            render_planilha_final_op()

    # Botões de navegação
    col1, col2, col3 = st.columns(3)
    with col1:
        if st.session_state.current_page > 0 and st.button("Anterior", key="prev_button"):
